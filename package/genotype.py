...
def read_source_table_stratification_cohorts_models(
    ...
    table = pandas.read_csv(
        ...
        dtype={
            ...
            "variables_selection": "string", # variables to select non-missing
            "variables_prefix_selection": "string", # variables by prefix
...
def extract_stratification_record_parameters(
    ...
    pail["white_british"] = extract_variable_values(
        field=record["white_british"]
    )
    pail["sex_text"] = extract_variable_values(
        field=record["sex_text"]
    )
    pail["female_age_grade"] = extract_variable_values(
        field=record["female_age_grade"]
    )
    pail["female_menopause"] = extract_variable_values(
        field=record["female_menopause"]
    )
    pail["female_menstruation"] = extract_variable_values(
        field=record["female_menstruation"]
    )
    pail["female_pregnancy"] = extract_variable_values(
        field=record["female_pregnancy"]
    )
    pail["male_age_grade"] = extract_variable_values(
    ...
    pail["dependence"] = extract_variable_values(
    ...
    pail["variables_selection"] = extract_variable_values(
        field=record["variables_selection"]
    )
    pail["variables_prefix_selection"] = extract_variable_values(
        field=record["variables_prefix_selection"]
    )
    pail["independence"] = extract_variable_values(
        field=record["independence"]
    )
    pail["independence_prefix"] = extract_variable_values(
...
# 1. read in parameter table with one record (row) for each stratification table
...
# - - this step will probably be easier if I don't have values of None... just include all allowable values for stratification
# 5. recombine records for female and male sex
# 6. select columns from union of "dependence", "variables_selection", and "variables_prefix_selection"
# 7. select records by non-missing values in union of "dependence", "variables_selection", and "variables_prefix_selection"
...
def execute_procedure(
    ...
    for record_stratification in records_stratification:
        ...
        record_cohort = control_stratify_genotype_cohorts(
            ...
            variables_selection=pail["variables_selection"],
            variables_prefix_selection=pail["variables_prefix_selection"],
